...
from SimEngine.Mote.sf import SchedulingFunctionMSF, SchedulingFunctionTREE
...
class Tsch(object):
    ...
    def enqueue(self, packet, priority=False):
        ...
        # TREE
        if u'mac' in packet.keys():
            self.mote.sf.indication_packet_enqueue(dest_addr=packet[u'mac'][u'dstMac'], success=goOn)
    ...
    def dequeue(self, packet):
        ...
        if (
                packet[u'mac'][u'dstMac'] != d.BROADCAST_ADDRESS
                and
                isinstance(self.mote.sf, SchedulingFunctionTREE)
                and
                not [
                    _pkt for _pkt in self.txQueue
                    if _pkt[u'mac'][u'dstMac'] == packet[u'mac'][u'dstMac']
            ...
            if (
                ...
                for packet in self.txQueue:
                    ...
                    for _, slotframe in list(self.slotframes.items()):
                        ...
                        # if isinstance(self.mote.sf, SchedulingFunctionTREE): dedicated_tx_cells = [] #TREE
