...
class RplOFStaticLinkPDR(RplOF0):
    ACCEPTABLE_LOWEST_PDR  = 0.1
    INVALID_RSSI_VALUE = -1000
    NONE_PREFERRED_PARENT = {
        u'mac_addr': None,
        u'mote_id': None,
    ...
    def __init__(self, rpl):
        super(RplOFStaticLinkPDR, self).__init__(rpl)
        self.preferred_parent = self.NONE_PREFERRED_PARENT
        self.neighbors = []
        self.path_pdr = 0
    ...
    def _update_preferred_parent(self):
        ...
        #TREE only change if we have no pref
        if new_preferred_parent != self.preferred_parent and \
                self.preferred_parent == self.NONE_PREFERRED_PARENT and \
                new_preferred_parent != self.NONE_PREFERRED_PARENT:
            if (
                    (new_preferred_parent == self.NONE_PREFERRED_PARENT)
                    or
