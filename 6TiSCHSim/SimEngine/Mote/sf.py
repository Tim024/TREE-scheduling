...
class SchedulingFunctionBase(object):
    ...
    @abstractmethod
    def indication_tx_cell_elapsed(self, cell, sent_packet, used):
    ...
    @abstractmethod
    def indication_packet_enqueue(self, dest_address, success):
        raise NotImplementedError()  # abstractmethod
...
class SchedulingFunctionTREE(SchedulingFunctionBase):
    SLOTFRAME_HANDLE = 1
    SLOTFRAME_AUTONOMOUS_HANDLE = 2
    ...
    def __init__(self, mote):
        ...
        super(SchedulingFunctionTREE, self).__init__(mote)
        ...
        # HYPERPARAM
        self.PSI_INC = 2000
        self.EVAP_PERIOD = 50  # 100 #50 #in slot amount #if we double sf size, we double evap period
        self.PSI_EVAP_COEF = 0.99
        self.PHI_EVAP_COEF = 0.99
        self.PSI_PUNISH_COEF = 0.3
        self.PHI_INC = 2000
        self.PHI_THRESHOLD = 9000
        self.PSI_THREHSOLD = 30
        self.PSI_INIT = 10000
        self.Q_FACTOR = 0.3

        # since we compare wih RPL upstream, only schedule with parent
        # else, create such a structure for each neighbor
        self.PHI_parent = 0
        self.PSI = {}  # key = custom dict upon creation
    ...
    def start(self):
        ...
        # use same autonomous SF as MSF
        self.mote.tsch.add_slotframe(
            slotframe_handle=self.SLOTFRAME_AUTONOMOUS_HANDLE,
            length=101
        )
        ...
        # start periodic evaporation of variables
        self._evap_all()
    ...
    def stop(self):
        ...
        self.mote.tsch.delete_slotframe(self.SLOTFRAME_AUTONOMOUS_HANDLE)

        self.engine.removeFutureEvent(
            (self.mote.id, u'_evap_all')
        )
    ...
    def _evap_all(self):
        ...
        for k, v in self.PSI.items():
            self.PSI[k]['psi'] = self.PSI[k]['psi'] * self.PSI_EVAP_COEF
            debug_str += " PSI " + str(self.PSI[k])
            if self.PSI[k]['psi'] <= self.PSI_THREHSOLD:
                if self.mote.tsch.get_cell(
                        slot_offset=k[0],
                        channel_offset=k[1],
                        mac_addr=self.PSI[k]['addr'],
                        slotframe_handle=self.SLOTFRAME_HANDLE
                ) is None:
                    ...
                    self.mote.tsch.deleteCell(
                        slotOffset=k[0],
                        channelOffset=k[1],
                        neighbor=self.PSI[k]['addr'],
                        cellOptions=self.PSI[k]['opt'],
                        slotframe_handle=self.SLOTFRAME_HANDLE
                    )
                to_remove_from_list.append(k)

        for k in to_remove_from_list:
            del self.PSI[k]

        if len(to_remove_from_list) > 0:
            debug_str += " DELETED " + str(to_remove_from_list)

        self.PHI_parent = self.PHI_parent * self.PHI_EVAP_COEF

        debug_str += " PHI=" + str(self.PHI_parent)
        self.log(SimEngine.SimLog.LOG_TREE, {'_mote_id': self.mote.id, 'debug': debug_str})

        self.engine.scheduleAtAsn(
            asn=self.engine.asn + self.EVAP_PERIOD,
            cb=self._evap_all,
            uniqueTag=(self.mote.id, '_evap_all'),
            intraSlotOrder=d.INTRASLOTORDER_STACKTASKS,
        )

    def get_tx_cells(self, addr):
        slotframe = self.mote.tsch.get_slotframe(
            self.SLOTFRAME_HANDLE
        )
        if slotframe:
            cells = slotframe.get_cells_by_mac_addr(addr)
            negotiated_tx_cells = [cell for cell in cells
                                   if cell.options == [d.CELLOPTION_TX]]
            autonomous_tx_cell = self.get_autonomous_tx_cell(addr)
    ...
    # === indications from other layers
    # 42  1863230323460158165,
    # 42 7735802574202388764,
    # 61 5296171221955769993,
    # 61 7573157399920786180,
    # pb 51 5161831117101838867,
    # pb 51 4523436727364872202,
    # 81 7366485168867120058,
    def indication_packet_enqueue(self, dest_addr, success):
        preferred_parent = self.mote.rpl.getPreferredParent()
        if success and dest_addr != d.BROADCAST_ADDRESS and not self.get_tx_cells(dest_addr):
            self.mote.sf.allocate_autonomous_tx_cell(dest_addr)

        if dest_addr == preferred_parent and dest_addr != None:
            num_tx_cells = len(self.get_tx_cells(dest_addr))
            num_p_queue = self.Q_FACTOR * self.mote.tsch.get_num_packet_in_tx_queue(dest_addr) * self.PHI_INC
            self.PHI_parent += (self.PHI_INC + num_p_queue) / float(1 + num_tx_cells)
            self.log(SimEngine.SimLog.LOG_TREE, {'_mote_id': self.mote.id, 'debug': "PHI INC " + str(
                (self.PHI_INC + num_p_queue) / float(1 + num_tx_cells)) + ' ' + str(num_p_queue) + ' ' + str(
                num_tx_cells)})

            # We need to add a cell here
            if self.PHI_parent >= self.PHI_THRESHOLD:
                self.retry_count[dest_addr] = 0
                self._request_adding_cells(dest_addr, num_tx_cells=5)
                self.PHI_parent = self.PHI_parent / 2
        else:
            self.log(SimEngine.SimLog.LOG_TREE, {'_mote_id': self.mote.id, 'debug': "PHI INC INCORRECT DEST ADDRESS" + str(dest_addr)})

    def indication_neighbor_added(self, neighbor_mac_addr):
        pass

    def indication_tx_cell_elapsed(self, cell, sent_packet, used):
        k = (cell.slot_offset, cell.channel_offset)
        if sent_packet is not None:
            if u'mac' in sent_packet.keys():
                is_broadcast = sent_packet[u'mac'][u'dstMac'] == d.BROADCAST_ADDRESS
                dest_addr = sent_packet[u'mac'][u'dstMac']
                preferred_parent = self.mote.rpl.getPreferredParent()
                if dest_addr == preferred_parent:
                    # if k != (0,0) and k != self._get_autonomous_cell(dest_addr):
                    if d.CELLOPTION_SHARED not in cell.options:
                        if (used):
                            self.PSI[k]['psi'] = self.PSI[k]['psi'] + self.PSI_INC
                        elif not (is_broadcast):
                            self.PSI[k]['psi'] = self.PSI[k]['psi'] * self.PSI_PUNISH_COEF
                        self.log(SimEngine.SimLog.LOG_TREE,
                                 {'_mote_id': self.mote.id, 'debug': "INC_PSI TX " + str(self.PSI)})

    def indication_rx_cell_elapsed(self, cell, received_packet):
        if received_packet is not None:
            if u'mac' in received_packet.keys():
                addr = received_packet[u'mac'][u'srcMac']
                is_broadcast = addr == d.BROADCAST_ADDRESS
                co = cell.channel_offset
                ts = cell.slot_offset
                # if not is_broadcast and (ts,co) != (0,0) and (ts,co)!=self._get_autonomous_cell(self.mote.get_mac_addr()):
                if not is_broadcast and d.CELLOPTION_SHARED not in cell.options:
                    k = (ts, co)
                    self.PSI[k]['psi'] = self.PSI[k]['psi'] + self.PSI_INC
                    self.log(SimEngine.SimLog.LOG_TREE,
                             {'_mote_id': self.mote.id, 'debug': "INC_PSI RX " + str(self.PSI)})
    ...
    def clear_to_send_EBs_DATA(self):
        return self.mote.dagRoot or self.mote.rpl.getPreferredParent() is not None
    ...
    def allocate_autonomous_tx_cell(self, mac_addr):
        ...
        # k = (slot_offset, channel_offset)
        # self.PSI[k] = {'addr': mac_addr, 'opt': [d.CELLOPTION_TX], 'psi': self.PSI_INIT}
    ...
    def _add_cells(self, neighbor, cell_list, cell_options):
        try:
            for cell in cell_list:
                ...
                k = (cell['slotOffset'], cell['channelOffset'])
                self.PSI[k] = {'addr': neighbor, 'opt': cell_options, 'psi': self.PSI_INIT}

                debug_str = " CREATED CELL " + str(self.PSI[k])
                self.log(SimEngine.SimLog.LOG_TREE, {'_mote_id': self.mote.id, 'debug': debug_str})
    ...
    def get_autonomous_tx_cell(self, mac_addr):
        slotframe = self.mote.tsch.get_slotframe(
            self.SLOTFRAME_AUTONOMOUS_HANDLE
        )
        if slotframe:
            ...
            if autonomous_cells:
                assert len(autonomous_cells) == 1, "ERR" + str(mac_addr) + ' ' + str(autonomous_cells)
    ...
    def _create_occupied_cell_list(
        ...
        occupied_cells = [cell for cell in self.mote.tsch.get_cells(neighbor, self.SLOTFRAME_HANDLE) if
                          cell.options == cell_options]
    ...
    def _are_cells_allocated(
        ...
        allocated_cells = [cell for cell in self.mote.tsch.get_cells(peerMac, self.SLOTFRAME_HANDLE) if
                           cell.options == cell_options]
...
class SchedulingFunctionMSF(SchedulingFunctionBase):
    ...
    def _housekeeping_collision(self):
        ...
        tx_cell_list = [cell for cell in
                        self.mote.tsch.get_cells(preferred_parent, self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS) if
                        cell.options == [d.CELLOPTION_TX]]
    ...
    def _create_occupied_cell_list(
        ...
        occupied_cells = [cell for cell in self.mote.tsch.get_cells(neighbor, self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS) if
                          cell.options == cell_options]
    ...
    def _are_cells_allocated(
        ...
        # collect allocated cells
        assert cell_options in [self.TX_CELL_OPT, self.RX_CELL_OPT]
        allocated_cells = [cell for cell in self.mote.tsch.get_cells(peerMac, self.SLOTFRAME_HANDLE_NEGOTIATED_CELLS) if
                           cell.options == cell_options]
